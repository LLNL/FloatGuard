    ...
    while True:
        ...
        if len(output) >= 3:
            if output[2].startswith("$") and "__device_stub__" in output[2]:
                ...
                if m:
                    kernel_name = m.group(1).replace("__device_stub__", "")
                    kernel_seq.append(kernel_name)

        saved_rip = "dummy"          

        if len(kernel_seq) > len(orig_kernel_seq):
            output = send(gdb, "info", "frame").splitlines()
            for line in output:
                if "saved rip = " in line:
                    saved_rip = line.strip().split()[-1]
                    saved_rip_seq.append(saved_rip)                  
            fp_e = False
            for rip in saved_rips:
                if rip == saved_rip:
                    send_param(gdb, 0)
        ...
        if "received signal" in output:
            ...
            if "Arithmetic exception" in output:
                ...
                send(gdb, "bt", display=True)
                idx = len(kernel_seq) - 1
                while idx >= 0:
                    if kernel_seq[idx] in output:
                        saved_rips.append(saved_rip_seq[idx])
                        break
                    idx -= 1               
