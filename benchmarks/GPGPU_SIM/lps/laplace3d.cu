...
int main(int argc, char **argv){
  ...
  if( NX = getCmdLineArgumentInt( argc, (const char**)argv, "nx") ) {
    if( NX <= 99 ) {
  ...
  if( NY = getCmdLineArgumentInt( argc, (const char**)argv, "ny") ) {
    if( NY <= 99 ) {
  ...
  if( NZ = getCmdLineArgumentInt( argc, (const char**)argv, "nz") ) {
    if( NZ <= 99 ) {
  ...
  if( REPEAT = getCmdLineArgumentInt( argc, (const char**)argv, "repeat") ) {
    if( REPEAT <= 0 ) {
  ...
  sdkStopTimer(&hTimer);
  printf("\n%dx GPU_laplace3d: %f (ms) \n", REPEAT, sdkGetTimerValue(&hTimer));
